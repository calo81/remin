...
(function(){var i="application/json",h="addNewDocument",g="completed",f="documentAdded",e="GET",d="login",c="remin_qooxdoo.Application",b="userLogged",a="editDocument";
...
var l=new remin_qooxdoo.MainWindow();
var k=new remin_qooxdoo.LoginWindow();
var m=new remin_qooxdoo.DocumentForm();
k.addListener(b,function(){l.openAndFetchDocuments();
k.destroy();
});
l.addListener(h,function(){m.open();
});
l.addListener(a,function(event){m.openWithData(event.getData());
}),m.addListener(f,function(){l.openAndFetchDocuments();
m.close();
});
var j=new qx.io.remote.Request(d,e,i);
j.addListener(g,function(event){if(event.getContent().ok!=undefined){l.openAndFetchDocuments();
}else{k.open();
}},this);
j.send();
}}});
...
(function(){var o="dblclick",n="document",m="New",l="remin_qooxdoo.MainWindow",k="addNewDocument",j="application/json",i="Collections",h="completed",g="icon/16/actions/document-new.png",f="qx.event.type.Event",c="GET",e="remin",d="editDocument",b="execute",a="qx.event.type.Data";
qx.Class.define(l,{extend:qx.ui.window.Window,construct:function(){qx.ui.window.Window.call(this,e);
...
var r=new qx.ui.layout.Grid(0,0);
this.setLayout(r);
r.setColumnWidth(1,600);
var q=new qx.ui.tree.Tree();
q.setWidth(500);
q.setHeight(400);
this.add(q,{row:1,column:0,colSpan:2,rowSpan:2});
var p=new qx.ui.tree.TreeFolder(i);
q.setRoot(p);
p.setOpen(true);
this.tree=q;
},events:{"addNewDocument":f,"editDocument":a},members:{tree:{},addNewDocument:function(){this.fireEvent(k);
},getMenuButton:function(){var toolbar=new qx.ui.toolbar.ToolBar;
toolbar.setWidth(600);
var t=new qx.ui.toolbar.Part;
toolbar.add(t);
var s=new qx.ui.toolbar.Button(m,g,this.__newCommand);
t.add(s);
s.addListener(b,this.addNewDocument,this);
return toolbar;
},populateTree:function(event){theWindow=this;
var u=new Array();
event.getContent().forEach(function(document){if(u[document.category]==null){u[document.category]=new qx.ui.tree.TreeFolder(document.category);
}var w=u[document.category];
var v=new qx.ui.tree.TreeFile(document.name);
theWindow.tree.getRoot().add(w);
var x=document;
w.setOpen(true);
w.add(v);
v.addListener(o,function(){this.fireDataEvent(d,x);
},theWindow);
});
},openAndFetchDocuments:function(){var y=new qx.io.remote.Request(n,c,j);
y.addListener(h,this.populateTree,this);
...
(function(){var o="one",n="single",m="selected",k="additive",j="multi",h="os.name",g="osx",f="under",d="PageUp",c="Left",O="lead",N="Down",M="Up",L="Boolean",K="PageDown",J="anchor",I="End",H="Home",G="Right",F="right",v="click",w="above",t="left",u="Escape",r="A",s="Space",p="_applyMode",q="__mI",x="interval",y="changeSelection",A="qx.event.type.Data",z="quick",C="key",B="abstract",E="drag",D="qx.ui.core.selection.Abstract";
qx.Class.define(D,{type:B,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__mF={};
},events:{"changeSelection":A},properties:{mode:{check:[n,j,k,o],init:n,apply:p},drag:{check:L,init:false},quick:{check:L,init:false}},members:{__mG:0,__mH:0,__mI:null,__mJ:null,__mK:null,__mL:null,__mM:null,__mN:null,__mO:null,__mP:null,__mQ:null,__mR:null,__mS:null,__mT:null,__mU:null,__mV:null,__mW:null,__mF:null,__mX:null,__mY:null,_userInteraction:false,__na:null,getSelectionContext:function(){return this.__mV;
...
},getSelection:function(){return qx.lang.Object.getValues(this.__mF);
},getSortedSelection:function(){var bf=this.getSelectables();
var be=qx.lang.Object.getValues(this.__mF);
be.sort(function(a,b){return bf.indexOf(a)-bf.indexOf(b);
});
return be;
},isItemSelected:function(bg){var bh=this._selectableToHashCode(bg);
return this.__mF[bh]!==undefined;
},isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__mF);
...
},_setLeadItem:function(bk){var bl=this.__mW;

if(bl!==null){this._styleSelectable(bl,O,false);
}
if(bk!==null){this._styleSelectable(bk,O,true);
}this.__mW=bk;
},getLeadItem:function(){return this.__mW!==null?this.__mW:null;
},_setAnchorItem:function(bm){var bn=this.__mX;

if(bn){this._styleSelectable(bn,J,false);
}
if(bm){this._styleSelectable(bm,J,true);
}this.__mX=bm;
},_getAnchorItem:function(){return this.__mX!==null?this.__mX:null;
...
},handleMouseOver:function(event){if(this.__na!=null&&this.__na!=this._getScroll().top){this.__na=null;
return;
...
if(this.isItemSelected(bM)&&!bL&&!bO&&!this.getDrag()){this.__mY=bM;
this._userInteraction=false;
return;
}else{this.__mY=null;
...
if(this.getDrag()&&bP!==n&&bP!==o&&!bL&&!bO){this.__mM=this._getLocation();
this.__mJ=this._getScroll();
this.__mN=event.getDocumentLeft()+this.__mJ.left;
this.__mO=event.getDocumentTop()+this.__mJ.top;
this.__mP=true;
this._capture();
}this._fireChange(v);
this._userInteraction=false;
...
if(!bT&&!bQ&&this.__mY){var bR=this._getSelectableFromMouseEvent(event);
...
},handleMouseMove:function(event){if(!this.__mP){return;
}this.__mQ=event.getDocumentLeft();
this.__mR=event.getDocumentTop();
this._userInteraction=true;
var bV=this.__mQ+this.__mJ.left;

if(bV>this.__mN){this.__mS=1;
}else if(bV<this.__mN){this.__mS=-1;
}else{this.__mS=0;
}var bU=this.__mR+this.__mJ.top;

if(bU>this.__mO){this.__mT=1;
}else if(bU<this.__mO){this.__mT=-1;
}else{this.__mT=0;
}var location=this.__mM;

if(this.__mQ<location.left){this.__mG=this.__mQ-location.left;
}else if(this.__mQ>location.right){this.__mG=this.__mQ-location.right;
}else{this.__mG=0;
}
if(this.__mR<location.top){this.__mH=this.__mR-location.top;
}else if(this.__mR>location.bottom){this.__mH=this.__mR-location.bottom;
}else{this.__mH=0;
}if(!this.__mI){this.__mI=new qx.event.Timer(100);
this.__mI.addListener(x,this._onInterval,this);
}this.__mI.start();
this._autoSelect();
...
},_cleanup:function(){if(!this.getDrag()&&this.__mP){return;
}if(this.__mU){this._fireChange(v);
}delete this.__mP;
delete this.__mK;
delete this.__mL;
this._releaseCapture();
if(this.__mI){this.__mI.stop();
}},_onInterval:function(e){this._scrollBy(this.__mG,this.__mH);
this.__mJ=this._getScroll();
this._autoSelect();
},_autoSelect:function(){var cg=this._getDimension();
var bY=Math.max(0,Math.min(this.__mQ-this.__mM.left,cg.width))+this.__mJ.left;
var bX=Math.max(0,Math.min(this.__mR-this.__mM.top,cg.height))+this.__mJ.top;
if(this.__mK===bY&&this.__mL===bX){return;
}this.__mK=bY;
this.__mL=bX;
...
},__nb:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;
var cp,co;
...
}},__nc:function(cG){var cI={};
var cH;

for(var i=0,l=cG.length;i<l;i++){cH=cG[i];
cI[this._selectableToHashCode(cH)]=cH;
}return cI;
},_getSelectedItem:function(){for(var cJ in this.__mF){return this.__mF[cJ];
}return null;
},_setSelectedItem:function(cK){if(this._isSelectable(cK)){var cL=this.__mF;
...
this.__mU=true;
this._fireChange();
},_fireChange:function(dc){if(this.__mU){this.__mV=dc||null;
this.fireDataEvent(y,this.getSelection());
delete this.__mU;
}}},destruct:function(){this._disposeObjects(q);
this.__mF=this.__mY=this.__mX=null;
this.__mW=null;
}});
})();
...
this.__nd=g;
},members:{__nd:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__nd;
...
},_getLocation:function(){var p=this.__nd.getContentElement().getDomElement();
...
},_scrollBy:function(u,v){},_scrollItemIntoView:function(w){this.__nd.scrollChildIntoView(w);
},getSelectables:function(x){var y=false;

if(!x){y=this._userInteraction;
this._userInteraction=true;
}var B=this.__nd.getChildren();
var z=[];
var A;
...
},_getFirstSelectable:function(){var I=this.__nd.getChildren();

for(var i=0,l=I.length;i<l;i++){if(this._isItemSelectable(I[i])){return I[i];
}}return null;
},_getLastSelectable:function(){var J=this.__nd.getChildren();

for(var i=J.length-1;i>0;i--){if(this._isItemSelectable(J[i])){return J[i];
}}return null;
},_getRelatedSelectable:function(K,L){var O=this.__nd.getOrientation()===f;
var N=this.__nd.getChildren();
var M=N.indexOf(K);
var P;
...
}}},destruct:function(){this.__nd=null;
}});
})();
...
(function(){var f="change",d="qx.event.type.Data",c="qx.ui.form.MModelSelection",b="__ne",a="changeSelection";
qx.Mixin.define(c,{construct:function(){this.__ne=new qx.data.Array();
this.__ne.addListener(f,this.__nh,this);
this.addListener(a,this.__ng,this);
},events:{changeModelSelection:d},members:{__ne:null,__nf:false,__ng:function(){if(this.__nf){return;
...
}}},__nh:function(){this.__nf=true;
var n=this.getSelectables(true);
var p=[];
var o=this.__ne.toArray();
...
if(!qx.lang.Array.equals(q,p)){this.__ng();
}},getModelSelection:function(){return this.__ne;
},setModelSelection:function(t){if(!t){this.__ne.removeAll();
return;
}t.unshift(this.__ne.getLength());
t.unshift(0);
var u=this.__ne.splice.apply(this.__ne,t);
u.dispose();
}},destruct:function(){this._disposeObjects(b);
}});
})();
(function(){var v="single",u="Boolean",t="one",s="changeSelection",r="__ni",q="mouseup",p="mousedown",o="losecapture",n="multi",m="_applyQuickSelection",d="mouseover",l="_applySelectionMode",h="_applyDragSelection",c="qx.ui.core.MMultiSelectionHandling",b="removeItem",g="keypress",f="qx.event.type.Data",j="addItem",a="additive",k="mousemove";
qx.Mixin.define(c,{construct:function(){var x=this.SELECTION_MANAGER;
var w=this.__ni=new x(this);
...
},events:{"changeSelection":f},properties:{selectionMode:{check:[v,n,a,t],init:v,apply:l},dragSelection:{check:u,init:false,apply:h},quickSelection:{check:u,init:false,apply:m}},members:{__ni:null,selectAll:function(){this.__ni.selectAll();
...
}return this.__ni.isItemSelected(y);
...
}this.__ni.removeItem(A);
},selectRange:function(B,C){this.__ni.selectItemRange(B,C);
},resetSelection:function(){this.__ni.clearSelection();
...
if(!qx.lang.Array.equals(E,D)){this.__ni.replaceSelection(D);
}}},getSelection:function(){return this.__ni.getSelection();
},getSortedSelection:function(){return this.__ni.getSortedSelection();
},isSelectionEmpty:function(){return this.__ni.isSelectionEmpty();
},getSelectionContext:function(){return this.__ni.getSelectionContext();
},_getManager:function(){return this.__ni;
},getSelectables:function(F){return this.__ni.getSelectables(F);
},invertSelection:function(){this.__ni.invertSelection();
},_getLeadItem:function(){var G=this.__ni.getMode();

if(G===v||G===t){return this.__ni.getSelectedItem();
}else{return this.__ni.getLeadItem();
}},_applySelectionMode:function(H,I){this.__ni.setMode(H);
},_applyDragSelection:function(J,K){this.__ni.setDrag(J);
},_applyQuickSelection:function(L,M){this.__ni.setQuick(L);
...
this.addListener(q,function(){this.__nj={"x":0,"y":0};
},this);
this.__nj={};
this.__nk={};
}},properties:{appearance:{refine:true,init:v},width:{refine:true,init:100},height:{refine:true,init:200},scrollbarX:{check:[h,d,E],init:h,themeable:true,apply:c},scrollbarY:{check:[h,d,E],init:h,themeable:true,apply:c},scrollbar:{group:[x,t]}},members:{__nj:null,__nk:null,_createChildControlImpl:function(G,H){var I;
...
if(be&&bd){if(this.__nj[ba]==0){var bc=0;
}else{var bc=-(e[b+bb]()-this.__nj[ba]);
}this.__nj[ba]=e[b+bb]();
bd.scrollBy(bc);
if(this.__nk[ba]){clearTimeout(this.__nk[ba]);
this.__nk[ba]=null;
}this.__nk[ba]=setTimeout(qx.lang.Function.bind(function(bf){this.__nl(bf,ba);
},this,bc),100);
}},__nl:function(bg,bh){this.__nk[bh]=null;
var bj=this._isChildControlVisible(f+bh);

if(bg==0||!bj){return;
}if(bg>0){bg=Math.max(0,bg-3);
}else{bg=Math.min(0,bg+3);
}this.__nk[bh]=setTimeout(qx.lang.Function.bind(function(bk,bl){this.__nl(bk,bl);
},this,bg,bh),20);
...
qx.Mixin.define(d,{construct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().addListener(j,this.__nm,this);
...
},__nm:qx.core.Environment.select(l,{"true":function(e){var o=this.getInvalidMessage();
...
}},"false":null})},destruct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().removeListener(j,this.__nm,this);
}}});
})();
...
}else if(this.__nY.length>0){do{var M=this.__nY[0];
...
var K=false;
if(this.__nY.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__ob(M);
B+=F;
if(C&&this.__nY.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__nY.length>0);
}},__ob:function(N){N.setVisibility(w);
this.__nY.shift();
this.fireDataEvent(p,N);
},__oc:function(O){if(!O){return;
}this.__nY.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__oa.length-1;i>=0;i--){var P=this.__oa[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__oa[T]!=undefined){throw new Error("Priority already in use!");
}this.__oa[T]=S;
...
}for(var i=0;i<this.__nY.length;i++){this.__nY[i].setVisibility(w);
}this.__nY=[];
...
}},__od:false,_setAllowMenuOpenHover:function(bc){this.__od=bc;
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="__oe",o="blur",n="Enter",m="__og",l="Up",d="__of",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
},members:{__oh:null,__oi:null,__of:null,__og:null,__oe:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
},add:function(F){var G=this.__oe;
G.push(F);
F.setZIndex(1e6+G.length);
},remove:function(H){var I=this.__oe;

if(I){qx.lang.Array.remove(I,H);
}},hideAll:function(){var J=this.__oe;

if(J){for(var i=J.length-1;i>=0;i--){J[i].exclude();
}}},getActiveMenu:function(){var K=this.__oe;
...
}if(this.__oe.length>0&&!this._isInMenu(P)){this.hideAll();
...
},__on:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__oo:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__oo,this);
return;
...
if(this.__om){this.__om.call(this);
delete this.__om;
...
if(this.__ol){bo.cancelOpen(this.__ol);
this.__ol=null;
...
}if(this.__ol){br.cancelOpen(this.__ol);
...
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__op:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__op=[0,0,0,0];
...
}},__oq:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
(function(){var i="Integer",h="hovered",g="hover-button",f="__or",d="interval",c="mouseover",b="mouseout",a="qx.ui.form.HoverButton";
...
this.__or=new qx.event.AcceleratingTimer();
this.__or.addListener(d,this._onInterval,this);
},properties:{appearance:{refine:true,init:g},interval:{check:i,init:80},firstInterval:{check:i,init:200},minTimer:{check:i,init:20},timerDecrease:{check:i,init:2}},members:{__or:null,_onMouseOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;
}this.__or.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();
this.addState(h);
},_onMouseOut:function(e){this.__or.stop();
this.removeState(h);
...
s.addListener(b,this.__os,this);
this._add(s);
break;
...
},__os:function(){var w=this.getChildrenContainer().getChildren();
...
this.__ox=++qx.io.remote.Request.__ox;
},events:{"created":j,"configured":j,"sending":j,"receiving":j,"completed":P,"aborted":j,"failed":P,"timeout":P},statics:{__ox:0,methodAllowsRequestBody:function(T){return (T==O)||(T==I);
...
},init:true,apply:B},crossDomain:{check:k,init:false},fileUpload:{check:k,init:false},transport:{check:l,nullable:true},useBasicHttpAuth:{check:k,init:false},parseJson:{check:k,init:true}},members:{__ot:null,__ou:null,__ov:null,__ow:null,__ox:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
...
},__oy:qx.event.GlobalError.observeMethod(function(e){var U=e.clone();
U.setTarget(this);
...
},setRequestHeader:function(bf,bg){this.__ot[bf]=bg;
},removeRequestHeader:function(bh){delete this.__ot[bh];
},getRequestHeader:function(bi){return this.__ot[bi]||null;
},getRequestHeaders:function(){return this.__ot;
},setParameter:function(bj,bk,bl){if(bl){this.__ov[bj]=bk;
}else{this.__ou[bj]=bk;
}},removeParameter:function(bm,bn){if(bn){delete this.__ov[bm];
}else{delete this.__ou[bm];
}},getParameter:function(bo,bp){if(bp){return this.__ov[bo]||null;
}else{return this.__ou[bo]||null;
}},getParameters:function(bq){return (bq?this.__ov:this.__ou);
},setFormField:function(br,bs){this.__ow[br]=bs;
},removeFormField:function(bt){delete this.__ow[bt];
},getFormField:function(bu){return this.__ow[bu]||null;
},getFormFields:function(){return this.__ow;
},getSequenceNumber:function(){return this.__ox;
}},destruct:function(){this.setTransport(null);
this.__ot=this.__ou=this.__ov=this.__ow=null;
}});
})();
(function(){var t="Integer",s="aborted",r="_onaborted",q="_on",p="_applyEnabled",o="Boolean",n="sending",m="__oC",l="interval",k="__oA",c="failed",j="qx.io.remote.RequestQueue",g="timeout",b="completed",a="queued",f="io.maxrequests",d="receiving",h="singleton";
qx.Class.define(j,{type:h,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__oz=[];
this.__oA=[];
this.__oB=0;
this.__oC=new qx.event.Timer(500);
this.__oC.addListener(l,this._oninterval,this);
},properties:{enabled:{init:true,check:o,apply:p},maxTotalRequests:{check:t,nullable:true},maxConcurrentRequests:{check:t,init:qx.core.Environment.get(f)},defaultTimeout:{check:t,init:5000}},members:{__oz:null,__oA:null,__oB:null,__oC:null,getRequestQueue:function(){return this.__oz;
},getActiveQueue:function(){return this.__oA;
},_debug:function(){var u;
},_check:function(){this._debug();
if(this.__oA.length==0&&this.__oz.length==0){this.__oC.stop();
}if(!this.getEnabled()){return;
}if(this.__oz.length==0||(this.__oz[0].isAsynchronous()&&this.__oA.length>=this.getMaxConcurrentRequests())){return;
}if(this.getMaxTotalRequests()!=null&&this.__oB>=this.getMaxTotalRequests()){return;
}var v=this.__oz.shift();
...
w.send();
if(this.__oz.length>0){this._check();
}},_remove:function(x){qx.lang.Array.remove(this.__oA,x);
x.dispose();
this._check();
},__oD:0,_onsending:function(e){e.getTarget().getRequest()._onsending(e);
...
z[r](event);
}}catch(B){}}},_oninterval:function(e){var I=this.__oA;

if(I.length==0){this.__oC.stop();
return;
...
}this.__oC.setEnabled(J);
},add:function(L){L.setState(a);

if(L.isAsynchronous()){this.__oz.push(L);
}else{this.__oz.unshift(L);
}this._check();

if(this.getEnabled()){this.__oC.start();
}},abort:function(M){var N=M.getTransport();

if(N){N.abort();
}else if(qx.lang.Array.contains(this.__oz,M)){qx.lang.Array.remove(this.__oz,M);
...
},__oE:function(){var bR=this.getRequest();
...
}this.__oF=qx.bom.Iframe.create({id:I,name:I,src:K});
qx.bom.element.Style.set(this.__oF,z,A);
this.__oG=qx.bom.Element.create(o,{id:J,name:J,target:I});
qx.bom.element.Style.set(this.__oG,z,A);
qx.dom.Element.insertEnd(this.__oG,qx.dom.Node.getBodyElement(document));
this.__oH=qx.bom.Element.create(c,{id:b,name:b});
qx.dom.Element.insertEnd(this.__oH,this.__oG);
qx.dom.Element.insertEnd(this.__oF,qx.dom.Node.getBodyElement(document));
qx.event.Registration.addListener(this.__oF,a,this._onload,this);
this.__oI=qx.lang.Function.listener(this._onreadystatechange,this);
qx.bom.Event.addNativeListener(this.__oF,E,this.__oI);
...
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__oH:null,__oJ:0,__oG:null,__oF:null,__oI:null,send:function(){var M=this.getMethod();
var O=this.getUrl();
...
this.__oG.appendChild(Q);
}this.__oG.action=O;
this.__oG.method=M;
this.__oH.appendChild(document.createTextNode(this.getData()));
this.__oG.submit();
this.setState(r);
},_onload:qx.event.GlobalError.observeMethod(function(e){if(qx.core.Environment.get(j)==u&&this.getIframeHtmlContent()==k){return;
}
if(this.__oG.src){return;
}this._switchReadyState(qx.io.remote.transport.Iframe._numericMap.complete);
}),_onreadystatechange:qx.event.GlobalError.observeMethod(function(e){this._switchReadyState(qx.io.remote.transport.Iframe._numericMap[this.__oF.readyState]);
}),_switchReadyState:function(T){switch(this.getState()){case D:case s:case p:case v:this.warn("Ignore Ready State Change");
return;
}while(this.__oJ<T){this.setState(qx.io.remote.Exchange._nativeMap[++this.__oJ]);
}},setRequestHeader:function(U,V){},getResponseHeader:function(W){return null;
},getResponseHeaders:function(){return {};
},getStatusCode:function(){return 200;
},getStatusText:function(){return k;
},getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__oF);
},getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__oF);
},getIframeBody:function(){return qx.bom.Iframe.getBody(this.__oF);
...
},destruct:function(){if(this.__oF){qx.event.Registration.removeListener(this.__oF,a,this._onload,this);
qx.bom.Event.removeNativeListener(this.__oF,E,this.__oI);
if((qx.core.Environment.get(j)==y)){this.__oF.src=qx.util.ResourceManager.getInstance().toUri(w);
}qx.dom.Element.remove(this.__oF);
}
if(this.__oG){qx.dom.Element.remove(this.__oG);
}this.__oF=this.__oG=this.__oH=null;
}});
})();
...
},__oK:function(E){var F=function(){qx.bom.Event.removeNativeListener(E,g,F);
...
for(F=z.length;F>3;F-=3){D=t+qx.locale.Number.getGroupSeparator(this.__oL)+z.substring(F-3,F)+D;
}D=z.substring(0,F)+D;
}var B=this.getPrefix()?this.getPrefix():t;
var y=this.getPostfix()?this.getPostfix():t;
var E=B+(A?r:t)+D;

if(C.length>0){E+=t+qx.locale.Number.getDecimalSeparator(this.__oL)+C;
}E+=y;
return E;
},parse:function(I){var N=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__oL)+t);
var L=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__oL)+t);
...
(function(){var n=",",m="",k='"',j="string",h="null",g=':',f="engine.name",e="qx.jsonDebugging",d='-',c='\\u00',Q="__oR",P="new Date(Date.UTC(",O="__pa",N='\\\\',M='\\f',L="__oO",K='\\"',J="))",I='T',H="}",u='(',v='.',s="{",t='\\r',q="__oP",r=":",o='\\t',p="]",w="opera",x="[",A="mshtml",z="qx.jsonEncodeUndefined",C="__oY",B='\\b',E="qx.util.Json",D='Z"',y=')',G='\\n',F="__oQ";
qx.Class.define(E,{statics:{__oM:null,BEAUTIFYING_INDENT:"  ",BEAUTIFYING_LINE_END:"\n",CONVERT_DATES:null,__oN:{"function":L,"boolean":q,"number":F,"string":Q,"object":C,"undefined":O},NUMBER_FORMAT:new qx.util.format.NumberFormat(),__oO:function(R,S){return String(R);
},__oP:function(T,U){return String(T);
},__oQ:function(V,W){return isFinite(V)?String(V):h;
},__oR:function(X,Y){var ba;

if(/["\\\x00-\x1f]/.test(X)){ba=X.replace(/([\x00-\x1f\\"])/g,qx.util.Json.__oT);
}else{ba=X;
}return k+ba+k;
},__oS:{'\b':B,'\t':o,'\n':G,'\f':M,'\r':t,'"':K,'\\':N},__oT:function(a,b){var bb=qx.util.Json.__oS[b];

if(bb){return bb;
}bb=b.charCodeAt();
return c+Math.floor(bb/16).toString(16)+(bb%16).toString(16);
},__oU:function(bc,bd){var bf=[],bi=true,bh,be;
var bg=qx.util.Json.__pc;
bf.push(x);

if(bg){qx.util.Json.__oV+=qx.util.Json.BEAUTIFYING_INDENT;
bf.push(qx.util.Json.__oV);
}
for(var i=0,l=bc.length;i<l;i++){be=bc[i];
bh=this.__oN[typeof be];

if(bh){be=this[bh](be,i+m);

if(typeof be==j){if(!bi){bf.push(n);

if(bg){bf.push(qx.util.Json.__oV);
}}bf.push(be);
bi=false;
}}}
if(bg){qx.util.Json.__oV=qx.util.Json.__oV.substring(0,qx.util.Json.__oV.length-qx.util.Json.BEAUTIFYING_INDENT.length);
bf.push(qx.util.Json.__oV);
}bf.push(p);
return bf.join(m);
},__oW:function(bj,bk){if(!qx.util.Json.CONVERT_DATES){if(bj.toJSON&&qx.core.Environment.get(f)!=w&&qx.core.Environment.get(f)!=A){return k+bj.toJSON()+k;
}var bl=this.NUMBER_FORMAT;
bl.setMinimumIntegerDigits(2);
var bn=bj.getUTCFullYear()+d+bl.format(bj.getUTCMonth()+1)+d+bl.format(bj.getUTCDate())+I+bl.format(bj.getUTCHours())+g+bl.format(bj.getUTCMinutes())+g+bl.format(bj.getUTCSeconds())+v;
...
return P+bm+J;
}},__oX:function(bo,bp){var bs=[],bu=true,br,bq;
var bt=qx.util.Json.__pc;
bs.push(s);

if(bt){qx.util.Json.__oV+=qx.util.Json.BEAUTIFYING_INDENT;
bs.push(qx.util.Json.__oV);
}
for(var bp in bo){bq=bo[bp];
br=this.__oN[typeof bq];

if(br){bq=this[br](bq,bp);

if(typeof bq==j){if(!bu){bs.push(n);

if(bt){bs.push(qx.util.Json.__oV);
}}bs.push(this.__oR(bp),r,bq);
bu=false;
}}}
if(bt){qx.util.Json.__oV=qx.util.Json.__oV.substring(0,qx.util.Json.__oV.length-qx.util.Json.BEAUTIFYING_INDENT.length);
bs.push(qx.util.Json.__oV);
}bs.push(H);
return bs.join(m);
},__oY:function(bv,bw){if(bv){if(qx.lang.Type.isFunction(bv.toJSON)&&bv.toJSON!==this.__oM){return this.__pb(bv.toJSON(bw),bw);
}else if(qx.lang.Type.isDate(bv)){return this.__oW(bv,bw);
}else if(qx.lang.Type.isArray(bv)){return this.__oU(bv,bw);
}else if(qx.lang.Type.isObject(bv)){return this.__oX(bv,bw);
}return m;
}return h;
},__pa:function(bx,by){if(qx.core.Environment.get(z)){return h;
}},__pb:function(bz,bA){return this[this.__oN[typeof bz]](bz,bA);
},stringify:function(bB,bC){this.__pc=bC;
this.__oV=this.BEAUTIFYING_LINE_END;
var bD=this.__pb(bB,m);
...
}}},environment:{"qx.jsonEncodeUndefined":true,"qx.jsonDebugging":false},defer:function(bI){bI.__oM=Date.prototype.toJSON;
}});
})();
...
var s=++qx.io.remote.transport.Script.__pd;

if(s>=2000000000){qx.io.remote.transport.Script.__pd=s=1;
}this.__pe=null;
this.__pd=s;
},statics:{__pd:0,_instanceRegistry:{},ScriptTransport_PREFIX:f,ScriptTransport_ID_PARAM:b,ScriptTransport_DATA_PARAM:h,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[m,l,o]},isSupported:function(){return true;
...
}})},members:{__pf:0,__pe:null,__pd:null,send:function(){var x=this.getUrl();
x+=(x.indexOf(p)>=0?r:p)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+q+this.__pd;
...
}qx.io.remote.transport.Script._instanceRegistry[this.__pd]=this;
this.__pe=document.createElement(c);
this.__pe.charset=e;
this.__pe.src=x;
document.body.appendChild(this.__pe);
},_switchReadyState:function(B){switch(this.getState()){case n:case a:case d:case g:this.warn("Ignore Ready State Change");
return;
}while(this.__pf<B){this.setState(qx.io.remote.Exchange._nativeMap[++this.__pf]);
...
},destruct:function(){if(this.__pe){delete qx.io.remote.transport.Script._instanceRegistry[this.__pd];
document.body.removeChild(this.__pe);
}this.__pe=this._responseContent=null;
}});
})();
...
}},properties:{parseJson:{check:C,init:true}},members:{__pg:false,__ph:0,__pi:null,getRequest:function(){if(this.__pi===null){this.__pi=qx.io.remote.transport.XmlHttp.createRequestObject();
this.__pi.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
}return this.__pi;
},send:function(){this.__ph=0;
...
if(bk==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bk,this.__pg)){if(this.getState()===b){this.setState(g);
}this.failed();
return;
}}if(bk==3&&this.__ph==1){this.setState(qx.io.remote.Exchange._nativeMap[++this.__ph]);
}while(this.__ph<bk){this.setState(qx.io.remote.Exchange._nativeMap[++this.__ph]);
...
if(qx.io.remote.Exchange.wasSuccessful(bF,bG,this.__pg)){try{bH=this.getRequest().responseXML;
...
},members:{__pj:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__pk()){this.__pj.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});
}this.__pj[this._groupCounter].items.push(c);
this.__pj[this._groupCounter].labels.push(d);
this.__pj[this._groupCounter].options.push(g);
if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);
}this.__pj[this._groupCounter].names.push(name);
this._validationManager.add(c,e,f);
this._resetter.add(c);
},addGroupHeader:function(h,k){if(!this.__pk()){this._groupCounter++;
}this.__pj.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});
},addButton:function(l,m){this._buttons.push(l);
this._buttonOptions.push(m||null);
},__pk:function(){return this.__pj.length===0;
...
},events:{"changeValid":a,"complete":f},properties:{validator:{check:d,init:null,nullable:true},invalidMessage:{check:h,init:k},requiredFieldMessage:{check:h,init:k},context:{nullable:true}},members:{__pl:null,__pn:null,__pm:null,__po:null,add:function(l,m,n){if(!this.__pt(l)){throw new Error("Added widget not supported.");
}if(this.__pu(l)){if(m!=null){throw new Error("Widgets supporting selection can only be validated "+"in the form validator");
}}var o={item:l,validator:m,valid:null,context:n};
this.__pl.push(o);
},remove:function(p){var q=this.__pl;
...
for(var i=0;i<this.__pl.length;i++){s.push(this.__pl[i].item);
}return s;
},validate:function(){var x=true;
this.__po=true;
var u=[];
for(var i=0;i<this.__pl.length;i++){var v=this.__pl[i].item;
var y=this.__pl[i].validator;
u.push(v);
if(y==null){var t=this.__pp(v);
x=x&&t;
this.__po=t&&this.__po;
continue;
}var t=this.__pq(this.__pl[i],v.getValue());
x=t&&x;

if(t!=null){this.__po=t&&this.__po;
}}var w=this.__pr(u);

if(qx.lang.Type.isBoolean(w)){this.__po=w&&this.__po;
}x=w&&x;
this.__pv(x);

if(qx.lang.Object.isEmpty(this.__pm)){this.fireEvent(j);
}return x;
},__pp:function(z){if(z.getRequired()){if(this.__pu(z)){var A=!!z.getSelection()[0];
...
},__pq:function(D,E){var J=D.item;
var I=D.context;
var H=D.validator;
if(this.__ps(H)){this.__pm[J.toHashCode()]=null;
...
D.valid=G;
return G;
},__pr:function(K){var M=this.getValidator();
var N=this.getContext()||this;

if(M==null){return true;
}this.setInvalidMessage(k);

if(this.__ps(M)){this.__pm[this.toHashCode()]=null;
...
},__ps:function(P){var Q=false;

if(!qx.lang.Type.isFunction(P)){Q=qx.Class.isSubClassOf(P.constructor,qx.ui.form.validation.AsyncValidator);
}return Q;
},__pt:function(R){var S=R.constructor;
return qx.Class.hasInterface(S,qx.ui.form.IForm);
},__pu:function(T){var U=T.constructor;
return qx.Class.hasInterface(U,qx.ui.core.ISingleSelection);
},__pv:function(V){var W=this.__pn;
this.__pn=V;
if(W!=V){this.fireDataEvent(g,V,W);
}},getValid:function(){return this.__pn;
},isValid:function(){return this.getValid();
},getInvalidMessages:function(){var X=[];
for(var i=0;i<this.__pl.length;i++){var Y=this.__pl[i].item;

if(!Y.getValid()){X.push(Y.getInvalidMessage());
}}if(this.getInvalidMessage()!=k){X.push(this.getInvalidMessage());
}return X;
},reset:function(){for(var i=0;i<this.__pl.length;i++){var ba=this.__pl[i];
ba.item.setValid(true);
}this.__pn=null;
},setItemValid:function(bb,bc){this.__pm[bb.toHashCode()]=bc;
bb.setValid(bc);
this.__pw();
},setFormValid:function(bd){this.__pm[this.toHashCode()]=bd;
this.__pw();
},__pw:function(){var bf=this.__po;
for(var bg in this.__pm){var be=this.__pm[bg];
bf=be&&bf;
if(be==null){return;
}}this.__pv(bf);
this.__pm={};
...
this.__px=b;
},members:{__px:null,__py:null,__pz:null,__pA:null,validate:function(c,d,e,f){this.__pA=false;
this.__py=c;
this.__pz=e;
this.__px.call(f||this,this,d);
},validateForm:function(g,h,i){this.__pA=true;
this.__pz=h;
this.__px.call(i,g,this);
},setValid:function(j,k){if(this.__pA){if(k!==undefined){this.__pz.setInvalidMessage(k);
}this.__pz.setFormValid(j);
}else{if(k!==undefined){this.__py.setInvalidMessage(k);
}this.__pz.setItemValid(this.__py,j);
}}},destruct:function(){this.__pz=this.__py=null;
}});
})();
(function(){var a="qx.ui.form.Resetter";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__pB=[];
},members:{__pB:null,add:function(b){if(this._supportsValue(b)){var c=b.getValue();
}else if(this.__pE(b)){var c=b.getSelection();
}else{throw new Error("Item "+b+" not supported for reseting.");
}this.__pB.push({item:b,init:c});
},reset:function(){for(var i=0;i<this.__pB.length;i++){var d=this.__pB[i];
this.__pC(d.item,d.init);
}},resetItem:function(e){var f;

for(var i=0;i<this.__pB.length;i++){var g=this.__pB[i];

if(g.item===e){f=g.init;
break;
}}if(f===undefined){throw new Error("The given item has not been added.");
}this.__pC(e,f);
},__pC:function(h,j){if(this._supportsValue(h)){h.setValue(j);
}else if(this.__pE(h)){h.setSelection(j);
}},redefine:function(){for(var i=0;i<this.__pB.length;i++){var k=this.__pB[i].item;
this.__pB[i].init=this.__pD(k);
}},redefineItem:function(l){var m;

for(var i=0;i<this.__pB.length;i++){if(this.__pB[i].item===l){m=this.__pB[i];
break;
}}if(m===undefined){throw new Error("The given item has not been added.");
}m.init=this.__pD(m.item);
},__pD:function(n){if(this._supportsValue(n)){return n.getValue();
}else if(this.__pE(n)){return n.getSelection();
}},__pE:function(o){var p=o.constructor;
...
this.__pF=!qx.core.Environment.get(H)||(qx.core.Environment.get(i)==bb&&parseFloat(qx.core.Environment.get(l))>=2);
...
if(bk&&this.__pF){this.__pM().setStyles({"left":bl.left+bp,"top":bl.top+bp});
}
if(bm){if(this.__pF){this.__pM().setStyles({"width":innerWidth+bp,"height":innerHeight+bp});
...
if(this.__pF){if(bs){this._showPlaceholder();
...
if(this.__pF){this.__pM().setStyles(bx);
}if(bv){this.__pK=qx.bom.Label.getTextSize(m,bx);
...
}if(bD){this.fireDataEvent(be,bE,this.__pJ);
this.__pJ=bE;
if(this.getLiveUpdate()){this.__pL(bE);
}}},__pL:function(bG){var bH=this.__pI;
this.__pI=bG;

if(bH!=bG){this.fireNonBubblingEvent(o,qx.event.type.Data,[bG,bH]);
}},setValue:function(bI){if(bI===null){if(this.__pG){return bI;
}bI=j;
this.__pG=true;
...
}}},_removePlaceholder:function(){if(this.hasState(k)){this.__pM().setStyle(bc,X);
this.removeState(k);
}},_syncPlaceholder:function(){if(this.hasState(k)){this.__pM().setStyle(bc,L);
}},__pM:function(){if(this.__pH==null){this.__pH=new qx.html.Label();
var bR=qx.theme.manager.Color.getInstance();
this.__pH.setStyles({"visibility":X,"zIndex":6,"position":W,"color":bR.resolve(I)});
...
}},"false":null}),_applyPlaceholder:function(bS,bT){if(this.__pF){this.__pM().setValue(bS);
...
}},setEnabled:qx.core.Environment.select(k,{"webkit":function(u){this.__pP=u;

if(!u){this.setStyles({"userModify":d,"userSelect":j});
}else{this.setStyles({"userModify":null,"userSelect":this.__pO?null:j});
}},"default":function(v){this.setAttribute(b,v===false);
}}),setSelectable:qx.core.Environment.select(k,{"webkit":function(w){this.__pO=w;
qx.html.Element.prototype.setSelectable.call(this,this.__pP&&w);
...
},setWrap:function(B,C){if(this.__pN===l){this._setProperty(n,B,C);
}else{throw new Error("Text wrapping is only support by textareas!");
}return this;
},getWrap:function(){if(this.__pN===l){return this._getProperty(n);
}else{throw new Error("Text wrapping is only support by textareas!");
}}}});
})();
...
}},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__pQ:false,__pR:null,__pS:null,__pT:null,canHandleEvent:function(B,C){var D=B.tagName.toLowerCase();

if(C===m&&(D===m||D===d)){return true;
}
if(C===j&&(D===m||D===d||D===p)){return true;
}return false;
},registerEvent:function(E,F,G){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(b)<9&&qx.core.Environment.get(A)<9){if(!E.__pU){var H=E.tagName.toLowerCase();
var I=E.type;
...
qx.bom.Event.addNativeListener(E,a,this._onKeyPressWrapped);
}}}}},__pV:qx.core.Environment.select(k,{"mshtml":function(J){if(qx.core.Environment.get(b)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.addNativeListener(J,m,this._onInputWrapper);
...
}}),unregisterEvent:function(O,P){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(b)<9&&qx.core.Environment.get(A)<9){if(O.__pU){var Q=O.tagName.toLowerCase();
var R=O.type;
...
}}}},__pW:qx.core.Environment.select(k,{"mshtml":function(T){if(qx.core.Environment.get(b)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.removeNativeListener(T,m,this._onInputWrapper);
...
}}),_onKeyPress:qx.core.Environment.select(k,{"mshtml|opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__pS){this.__pS=Y.value;
qx.event.Registration.fireEvent(Y,j,qx.event.type.Data,[Y.value]);
}}},"default":null}),_inputFix:qx.core.Environment.select(k,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__pT){this.__pT=ba.value;
qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);
}}},"default":null}),_onKeyDown:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__pQ=true;
}},"default":null}),_onKeyUp:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__pQ=false;
}},"default":null}),_onBlur:qx.core.Environment.select(k,{"opera":function(e){if(this.__pR&&qx.core.Environment.get(t)<10.6){window.clearTimeout(this.__pR);
...
if(!this.__pQ||bb!==m){if((qx.core.Environment.get(k)==z)&&qx.core.Environment.get(t)<10.6){this.__pR=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);
},0);
...
qx.Class.define(h,{statics:{__pX:{text:1,textarea:1,select:1,checkbox:1,radio:1,password:1,hidden:1,submit:1,image:1,file:1,search:1,reset:1,button:1},create:function(w,x,y){var x=x?qx.lang.Object.clone(x):{};
var z;
...
}},properties:{model:{check:g,apply:a,event:h,nullable:true,dereference:true},target:{check:b,apply:f,event:d,nullable:true,init:null,dereference:true}},members:{__qa:null,__pY:null,addBindingOptions:function(name,o,p){this.__pY[name]=[o,p];
if(this.getModel()==null||this.getTarget()==null){return;
}var q=this.getTarget().getItems()[name];
var r=this.__qd(q)?k:j;
this.__qa.removeTarget(q,r,name);
this.__qa.addTarget(q,r,name,!this._selfUpdate,o,p);
...
}if(this.__pY[name]&&this.__pY[name][1]){t=this.__pY[name][1].converter(t);
}A[x[i]]=t;
}else{if(!A[x[i]]){A[x[i]]={};
}A=A[x[i]];
...
var D=this.__qd(E)?k:j;
var C=this.__pY[name];
C=C&&this.__pY[name][1];
qx.data.SingleValueBinding.updateTarget(E,D,this.getModel(),name,C);
}},_applyTarget:function(F,G){if(G!=null){this.__qc(G);
}if(this.getModel()==null){return;
}if(F!=null){this.__qb();
}},_applyModel:function(H,I){if(this.__qa!=null){var J=this.getTarget().getItems();

for(var name in J){var L=J[name];
var K=this.__qd(L)?k:j;
this.__qa.removeTarget(L,K,name);
}}if(this.__qa!=null){this.__qa.setModel(H);
}if(this.getTarget()==null){return;
}if(H!=null){this.__qb();
}},__qb:function(){if(this.__qa==null){this.__qa=new qx.data.controller.Object(this.getModel());
}var M=this.getTarget().getItems();
for(var name in M){var P=M[name];
var N=this.__qd(P)?k:j;
var O=this.__pY[name];
try{if(O==null){this.__qa.addTarget(P,N,name,!this._selfUpdate);
}else{this.__qa.addTarget(P,N,name,!this._selfUpdate,O[0],O[1]);
}}catch(Q){}}},__qc:function(R){if(this.__qa==null){return;
}var S=R.getItems();
for(var name in S){var U=S[name];
var T=this.__qd(U)?k:j;
this.__qa.removeTarget(U,T,name);
}},__qd:function(V){return qx.Class.hasInterface(V.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(V.constructor,qx.ui.form.IModelSelection);
}},destruct:function(){if(this.__qa){this.__qa.dispose();
}}});
})();
...
this.__qe=l;
},statics:{__qf:null,createModel:function(m,n){if(this.__qf===null){this.__qf=new qx.data.marshal.Json();
}this.__qf.toClass(m,n);
return this.__qf.toModel(m);
}},members:{__qe:null,__qg:function(o){var p=[];
...
if(this.__qe&&this.__qe.getModelClass&&this.__qe.getModelClass(u)!=null){return;
}for(var v in r){this.toClass(r[v],s);
}if(qx.Class.isDefined(k+u)){return;
}var B={};
var A={__qh:this.__qh};
...
if(s){B[v].apply=a;
}if(this.__qe&&this.__qe.getValidationRule){var x=this.__qe.getValidationRule(u,v);

if(x){B[v].validate=h+v;
A[h+v]=x;
}}}if(this.__qe&&this.__qe.getModelSuperClass){var z=this.__qe.getModelSuperClass(u)||qx.core.Object;
}else{var z=qx.core.Object;
}var w=[];

if(this.__qe&&this.__qe.getModelMixins){var y=this.__qe.getModelMixins(u);
if(!qx.lang.Type.isArray(y)){if(y!=null){w=[y];
}}}if(s){w.push(qx.data.marshal.MEventBubbling);
}var t={extend:z,include:w,properties:B,members:A,destruct:this.__qi};
qx.Class.define(k+u,t);
},__qi:function(){var C=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var D in C){this.__qh(this.get(C[D].name));
}},__qh:function(E){if(!(E instanceof qx.core.Object)){return ;
}if(E.isDisposed()){return;
}if(qx.Class.implementsInterface(E,qx.data.IListData)){for(var i=0;i<E.getLength();i++){this.__qh(E.getItem(i));
}}E.dispose();
},__qj:function(F){var G;
if(this.__qe&&this.__qe.getModelClass){G=this.__qe.getModelClass(F);
}
...
}else if(qx.lang.Type.isObject(I)){var J=this.__qg(I);
var N=this.__qj(J);
...
}},properties:{model:{check:f,event:g,apply:b,nullable:true,dereference:true}},members:{__ql:null,__qk:null,_applyModel:function(k,l){for(var i=0;i<this.__ql.length;i++){var t=this.__ql[i][0];
var q=this.__ql[i][1];
var o=this.__ql[i][2];
var r=this.__ql[i][3];
var s=this.__ql[i][4];
var n=this.__ql[i][5];
if(l!=undefined&&!l.isDisposed()){this.__qn(t,q,o,l);
}if(k!=undefined){this.__qm(t,q,o,r,s,n);
...
}}}}},addTarget:function(u,v,w,x,y,z){this.__ql.push([u,v,w,x,y,z]);
this.__qm(u,v,w,x,y,z);
},__qm:function(A,B,C,D,E,F){if(this.getModel()==null){return;
...
if(this.__qk[I]==undefined){this.__qk[I]=[];
}this.__qk[I].push([G,H,B,C,E,F]);
},removeTarget:function(J,K,L){this.__qn(J,K,L,this.getModel());
for(var i=0;i<this.__ql.length;i++){if(this.__ql[i][0]==J&&this.__ql[i][1]==K&&this.__ql[i][2]==L){this.__ql.splice(i,1);
}}},__qn:function(M,N,O,P){if(!(M instanceof qx.core.Object)){return ;
}var Q=this.__qk[M.toHashCode()];
...
B+=this.__qo(name,z,w);
}}else if(qx.lang.Type.isDate(y)&&x!=null){B+=this.__qo(name,x.format(y),w);
}else{B+=this.__qo(name,y,w);
}}return B.substring(0,B.length-1);
},__qo:function(name,D,E){if(D instanceof qx.core.Object&&E!=null){var F=encodeURIComponent(E(D));
...
(function(){var s="application/json",r="Name",q="document",p="Category",o="",n="name",m="no-url-params-on-post",l="Save",k="descriptionx",j="completed",d="remin",i="Content-Type",g="qx.event.type.Event",c="category",b="Description",f="remin_qooxdoo.DocumentForm",e="POST",h="execute",a="documentAdded";
...
this.addForm();
},events:{"documentAdded":g},properties:{docNameField:{},descriptionField:{},categoryField:{}},members:{openWithData:function(u){this.getDocNameField().setValue(u.name);
this.getCategoryField().setValue(u.category);
this.getDescriptionField().setValue(u.descriptionx);
this.open();
},addForm:function(){var w=new qx.ui.form.Form();
var x=this.getCategoryField();
x.setRequired(true);
w.add(x,p,null,c);
var B=this.getDocNameField();
B.setRequired(true);
w.add(B,r,null,n);
var v=this.getDescriptionField();
w.add(v,b,null,k);
var C=new qx.ui.form.Button(l);
w.addButton(C);
var A=new qx.ui.form.renderer.Single(w);
this.add(A,{row:0,column:0});
var z=new qx.data.controller.Form(null,w);
var D=z.createModel();
var E=new qx.ui.basic.Label(o);
this.add(E,{row:1,column:0});
var y=this;
C.addListener(h,function(){if(w.validate()){var F=new qx.io.remote.Request(q,e,s);
F.setData(qx.util.Serializer.toJson(z.getModel()));
F.setAsynchronous(false);
F.setCrossDomain(false);
F.setProhibitCaching(m);
F.setRequestHeader(i,s);
F.addListener(j,function(G){if(G.getContent().error!==undefined){E.setValue(G.getContent().error);
}else{y.fireEvent(a);
y.close();
}});
F.send();
...
},properties:{wrap:{check:g,init:true,apply:a},appearance:{refine:true,init:h},singleStep:{check:f,init:20},minimalLineHeight:{check:f,apply:w,init:4},autoSize:{check:g,apply:s,init:false}},members:{__qp:null,__qq:null,__qr:null,setValue:function(z){z=qx.ui.form.AbstractField.prototype.setValue.call(this,z);
this.__qs();
return z;
...
if(B!=scrollY){e.stop();
}},__qs:function(){if(this.isAutoSize()){var E=this.__qt();

if(E){this.__qr=this.__qr||this._getAreaHeight();
...
},_setAreaHeight:function(J){if(this._getAreaHeight()!==J){this.__qq=J;
qx.ui.core.queue.Layout.add(this);
qx.ui.core.queue.Manager.flush();
}},_getScrolledAreaHeight:function(){var L=this.__qt();
...
}},__qt:function(){this.__qp=this.__qp||this.__qu();
return this.__qp;
},__qu:function(){var Q,O,N,P;
...
this.__qv(O);
return O;
},__qv:function(R){var R=R.getDomElement();
...
},_applyAutoSize:function(U,V){if(U){this.__qs();
this.addListener(d,this.__qs,this);
this.addListenerOnce(c,function(){this.getContentElement().setStyle(k,i);
});
}else{this.removeListener(d,this.__qs);
...
if(W===this.getMaxHeight()){this.__qs();
}},__qw:function(){if(this.isAutoSize()&&this.getHeight()){this.warn("autoSize is ignored when the height property is set. "+"If you want to set an initial height, use the minHeight "+"property instead.");
...
if(this.isAutoSize()){X.height=this.__qq||X.height;
}return X;
}}});
})();
...
qx.Class.define(f,{statics:{__qx:/\s+/g,__qy:/^\s+|\s+$/g,add:qx.lang.Object.select(qx.core.Environment.get(s)?q:r,{"native":function(t,name){t.classList.add(name);
return name;
...
return M.className=M.className.replace(O,p).replace(this.__qy,p).replace(this.__qx,o);
...
if(bf&&bf.nodeType){return this.__qD([bf]);
}else{return bf;
}}return null;
};
};
...
},__qz:/^[^<]*(<(.|\s)+>)[^>]*$|^#([\w-]+)$/,create:function(bo,bp){var br=qx.bom.Collection;
if(bo.nodeType){return new br(bo);
}else if(typeof bo===I){var bq=br.__qz.exec(bo);
...
},__qB:/^.[^:#\[\.,]*$/,not:function(bH){if(this.__qB.test(bH)){var bI=qx.bom.Selector.matches(g+bH+d,this);
return this.__qD(bI);
...
return this.__qD(qx.lang.Array.unique(bS));
...
if(this.length===1){return this.__qD(ca.query(bX,this[0]));
}else{var bY=[];

for(var i=0,l=this.length;i<l;i++){bY.push.apply(bY,ca.query(bX,this[i]));
}return this.__qD(qx.lang.Array.unique(bY));
...
},nextAll:function(ce){return this.__qC(y,ce);
...
},prevAll:function(ci){return this.__qC(c,ci);
...
},parents:function(cl){return this.__qC(H,cl);
},siblings:function(cm){return this.__qC(s,cm);
},__qC:function(cn,co){var cq=[];
...
},__qD:function(cs){var ct=new qx.bom.Collection;
ct.__qA=this;
cs=Array.prototype.slice.call(cs,0);
ct.push.apply(ct,cs);
return ct;
},andSelf:function(){return this.add(this.__qA);
},end:function(){return this.__qA||new qx.bom.Collection();
},__qE:function(cu,cv){var cA=this[0];
...
},__qF:function(cF,cG){var cI=qx.bom.Selector;
...
},append:function(cL){return this.__qE(arguments,this.__qG);
},prepend:function(cM){return this.__qE(arguments,this.__qH);
},__qG:function(cN,cO){cN.appendChild(cO);
},__qH:function(cP,cQ){cP.insertBefore(cQ,cP.firstChild);
},appendTo:function(cR){return this.__qF(arguments,e);
},prependTo:function(cS){return this.__qF(arguments,J);
},before:function(cT){return this.__qE(arguments,this.__qI);
},after:function(cU){return this.__qE(arguments,this.__qJ);
},__qI:function(cV,cW){cV.parentNode.insertBefore(cW,cV);
},__qJ:function(cX,cY){cX.parentNode.insertBefore(cY,cX.nextSibling);
},insertBefore:function(da){return this.__qF(arguments,w);
},insertAfter:function(db){return this.__qF(arguments,q);
...
}dc.map(this.__qK).append(this);
}return this;
},__qK:function(de){while(de.firstChild){de=de.firstChild;
}return de;
...
},replaceAll:function(dh){return this.__qF(arguments,n);
...
qx.Class.define(n,{statics:{__qL:function(K,L,M){return M.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i)?K:L+z+M+H;
},__qM:{opt:[1,e,c],leg:[1,f,u],table:[1,h,y],tr:[2,r,E],td:[3,I,b],col:[2,G,C],def:qx.core.Environment.select(g,{"mshtml":[1,s,d],"default":null})},__qN:function(N,O){var U=O.createElement(J);
N=N.replace(/(<(\w+)[^>]*?)\/>/g,this.__qL);
...
if(typeof ba===m){ba=this.__qN(ba,X);
...
qx.Bootstrap.define(a,{construct:function(){this.__qO=qx.Bootstrap.bind(this.__qU,this);
this.__qP=document.createElement(f);
},members:{__qQ:null,__qR:null,__qS:null,__qT:null,__qO:null,__qP:null,load:function(q,r,s){if(this.__qQ){throw new Error("Another request is still running!");
}this.__qQ=true;
this.__qR=false;
...
this.__qT=s||window;
u.type=b;
u.onerror=u.onload=u.onreadystatechange=this.__qO;
u.src=q;
setTimeout(function(){t.appendChild(u);
},0);
},abort:function(){if(this.__qQ){this.dispose(d);
}},dispose:function(status){if(this.__qR){return;
}this.__qR=true;
var w=this.__qP;
...
if(this.__qS){if(qx.core.Environment.get(o)==i||qx.core.Environment.get(o)==c){var self=this;
setTimeout(qx.event.GlobalError.observeMethod(function(){self.__qS.call(self.__qT,status);
delete self.__qS;
}),0);
}else{this.__qS.call(this.__qT,status);
delete this.__qS;
}}},__qU:qx.event.GlobalError.observeMethod(qx.core.Environment.select(o,{"mshtml":function(e){var x=this.__qP.readyState;
...
qx.Mixin.define(o,{include:[qx.ui.decoration.MSingleBorder,qx.ui.decoration.MBackgroundImage],construct:function(){this._getDefaultInsetsForBorder=this.__ra;
this._resizeBorder=this.__qY;
this._styleBorder=this.__qW;
this._generateMarkup=this.__qX;
},properties:{innerWidthTop:{check:f,init:0},innerWidthRight:{check:f,init:0},innerWidthBottom:{check:f,init:0},innerWidthLeft:{check:f,init:0},innerWidth:{group:[w,z,n,k],mode:a},innerColorTop:{nullable:true,check:g},innerColorRight:{nullable:true,check:g},innerColorBottom:{nullable:true,check:g},innerColorLeft:{nullable:true,check:g},innerColor:{group:[u,v,x,r],mode:a}},members:{__qV:null,__qW:function(D){var E=qx.theme.manager.Color.getInstance();
D.position=t;
...
}},__qX:function(G){var J=this._generateBackgroundMarkup(G);
...
G[y]=0;
G[p]=0;
return this.__qV=this._generateBackgroundMarkup(G,J);
},__qY:function(K,L,M){var N=this.getInsets();
L-=N.left+N.right;
M-=N.top+N.bottom;
...
},__ra:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
...
}},members:{__rb:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__rb;
},getMarkup:function(e){if(this.__rb){return this.__rb;
...
}},properties:{innerColor:{check:d,nullable:true,apply:i},innerOpacity:{check:o,init:1,apply:i},outerColor:{check:d,nullable:true,apply:i}},members:{__rc:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
},_isInitialized:function(){return !!this.__rc;
},_applyStyle:function(){},getMarkup:function(){if(this.__rc){return this.__rc;
...
if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__rd=new qx.ui.decoration.css3.BorderImage();

if(n){this.__re(n);
}}else{this.__rd=new qx.ui.decoration.GridDiv(n);
}
if(o!=null){this.__rd.setInsets(o);
}},properties:{baseImage:{check:c,nullable:true,apply:h},insetLeft:{check:m,nullable:true,apply:l},insetRight:{check:m,nullable:true,apply:l},insetBottom:{check:m,nullable:true,apply:l},insetTop:{check:m,nullable:true,apply:l},insets:{group:[i,j,g,a],mode:e}},members:{__rd:null,getMarkup:function(){return this.__rd.getMarkup();
},resize:function(p,q,r){this.__rd.resize(p,q,r);
},tint:function(s,t){},getInsets:function(){return this.__rd.getInsets();
},_applyInsets:function(u,v,name){var w=f+qx.lang.String.firstUp(name);
this.__rd[w](u);
},_applyBaseImage:function(x,y){if(this.__rd instanceof qx.ui.decoration.GridDiv){this.__rd.setBaseImage(x);
}else{this.__re(x);
}},__re:function(z){var B,D;
this.__rd.setBorderImage(z);
...
}},statics:{IS_SUPPORTED:qx.bom.element.Style.isPropertySupported("borderImage")},properties:{borderImage:{check:t,nullable:true,apply:j},sliceTop:{check:h,init:0,apply:j},sliceRight:{check:h,init:0,apply:j},sliceBottom:{check:h,init:0,apply:j},sliceLeft:{check:h,init:0,apply:j},slice:{group:[n,v,a,w],mode:c},repeatX:{check:[i,e,d],init:i,apply:j},repeatY:{check:[i,e,d],init:i,apply:j},repeat:{group:[u,m],mode:c}},members:{__rf:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
...
this.__rf=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__rf;
...
}},properties:{baseImage:{check:q,nullable:true,apply:s}},members:{__rg:null,__rh:null,__ri:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
...
F.push(a);
return this.__rg=F.join(o);
},resize:function(G,H,I){var J=this.__ri;
...
var Q=S[1];
var P=S[2];
var O=this.__rh={tl:Q+v+P,t:Q+x+P,tr:Q+c+P,bl:Q+p+P,b:Q+r+P,br:Q+y+P,l:Q+b+P,c:Q+n+P,r:Q+u+P};
this.__ri=this._computeEdgeSizes(O);
